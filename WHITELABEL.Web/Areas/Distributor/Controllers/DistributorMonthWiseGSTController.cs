...
{
    [Authorize]
    public class DistributorMonthWiseGSTController : DistributorBaseController
    {
        ...
        // GET: Distributor/DistributorMonthWiseGST
        ...
        public JsonResult MonthlyGSTCalculateAmount(string MonthID = "", string ServiceName = "", string Year = "")
        {
            ...
            if (MonthID != null)
            {
                ...
                var GSTValu12e = (from x in db.TBL_ACCOUNTS
                                  where x.MEM_ID == MemberCurrentUser.MEM_ID && x.TRANSACTION_DATE.Month == Monthval && x.DR_CR == "CR" && x.TRANSACTION_TYPE == ServiceName && x.MEMBER_TYPE == "DISTRIBUTOR" && x.NARRATION == "Commission"
                                  select new
                                  {
                                      MEM_ID = x.MEM_ID,
                ...
                var getListGSTComm = db.TBL_ACCOUNTS.Where(x => x.MEM_ID == MemberCurrentUser.MEM_ID && x.TRANSACTION_DATE.Month == Monthval && x.DR_CR == "CR" && x.TRANSACTION_TYPE == ServiceName && x.MEMBER_TYPE == "DISTRIBUTOR" && x.NARRATION == "Commission").FirstOrDefault();
