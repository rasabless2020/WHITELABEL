...
{
    [Authorize]
    public class DistributorCommissionSettingController : DistributorBaseController
    {
        ...
        // GET: Distributor/DistributorCommissionSetting
        ...
        private IGrid<TBL_WHITE_LEVEL_COMMISSION_SLAB> CreateExportableGrid()
        {
            ...
            grid.Columns.Add(model => model.SLN).Titled("Edit").Encoded(false)
                .RenderedAs(model => "<div style='text-align:center;'><a href='" + @Url.Action("GenerateCommissionSlab", "DistributorCommissionSetting", new { area = "Distributor", SlabId = Encrypt.EncryptMe(model.SLN.ToString()) }) + "' Title='Edit'><i class='fa fa-edit'></i></a>");
        ...
        public JsonResult GetServiceProvider(string NewListId)
        {
            ...
            try
            {
                ...
                var WLInfo = db.TBL_MASTER_MEMBER.FirstOrDefault(x => x.MEM_ID == MemberCurrentUser.MEM_ID);
                if (NewListId == "MOBILE RECHARGE")
                {
                    ...
                    var OperatorList = (from WList in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB
                                        ...
                                        where WList.WHITE_LEVEL_ID == WLInfo.UNDER_WHITE_LEVEL && WList.INTRODUCER_ID == MemberCurrentUser.MEM_ID && slbmst.SLAB_TYPE == 1
                                        select new
                                        {
                                            ID = commPO.SLN,
                                            ...
                                            SuperDisPercentage = commPO.SUPER_COM_PER,
                                            DistriCommissionPer = commPO.DISTRIBUTOR_COM_PER,
                                            RetailerCommissionPer = commPO.MERCHANT_COM_PER
                                        }).AsEnumerable().Select(z => new CommissionListView
                                        {
                                            ID = z.ID,
                                            SERVICE_NAME = z.SERVICE_NAME,
                ...
                else if (NewListId == "UTILITY SERVICES")
                {
                    ...
                    var OperatorList = (from WList in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB
                                        ...
                                        //join commPO in db.TBL_COMM_SLAB_MOBILE_RECHARGE on WList.BILLPAYMENT_SLAB equals commPO.SLAB_ID
                                        ...
                                        where WList.WHITE_LEVEL_ID == WLInfo.UNDER_WHITE_LEVEL && WList.INTRODUCER_ID == MemberCurrentUser.MEM_ID && slbmst.SLAB_TYPE == 2
                                        select new
                                        {
                                            ID = commPO.SLN,
                                            SERVICE_NAME = commPO.OPERATOR_NAME,
                ...
                else if (NewListId == "DMR")
                {
                    ...
                    var OperatorList = (from WList in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB
                                        join commPO in db.TBL_COMM_SLAB_DMR_PAYMENT on WList.DMR_SLAB equals commPO.SLAB_ID
                                        join slbmst in db.TBL_WHITE_LEVEL_COMMISSION_SLAB on WList.DMR_SLAB equals slbmst.SLN
                                        //where WList.WHITE_LEVEL_ID == MemberCurrentUser.MEM_ID && WList.INTRODUCER_ID == 0 && slbmst.SLAB_TYPE == 1
                                        where WList.WHITE_LEVEL_ID == WLInfo.UNDER_WHITE_LEVEL && WList.INTRODUCER_ID == MemberCurrentUser.MEM_ID && slbmst.SLAB_TYPE == 3
                                        select new
                                        {
                                            ID = slbmst.SLN,
                                            SERVICE_NAME = commPO.DMT_TYPE,
                                            TYPE = "REMITTANCE",
                                            DMRFrom = commPO.SLAB_FROM,
                                            DMRTo = commPO.SLAB_TO,
                                            SERVICE_KEY = "DMI",
                                            COMM_TYPE = commPO.COMM_TYPE,
                                            CommissionPercentage = commPO.COMM_PERCENTAGE,
                                            SuperComm = commPO.SUPER_COM_PER,
                                            DistributorCom = commPO.DISTRIBUTOR_COM_PER,
                                            MerchantComm = commPO.MERCHANT_COM_PER,
                                            DistributorComm_Type = commPO.DISTRIBUTOR_COMM_TYPE,
                                            MerchantComm_Type = commPO.MERCHANT_COMM_TYPE,
                                        }).AsEnumerable().Select(z => new CommissionListView
                                        {
                                            ID = z.ID,
                                            SERVICE_NAME = z.SERVICE_NAME,
                    ...
                    ////                                           where dtlcom.WHITE_LEVEL_ID == WLInfo.UNDER_WHITE_LEVEL && dtlcom.INTRODUCER_ID == MemberCurrentUser.MEM_ID && DMcm.DMT_TYPE == "DOMESTIC"
        ...
        public JsonResult GetDMRCommissionValue()
        {
            ...
            {
                var db = new DBContext();
                var WLPInfo = db.TBL_MASTER_MEMBER.FirstOrDefault(x => x.MEM_ID == MemberCurrentUser.MEM_ID);
                var OperatorList = (from detail in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB
                                    join dmrcom in db.TBL_COMM_SLAB_DMR_PAYMENT on detail.DMR_SLAB equals dmrcom.SLAB_ID
                                    join white in db.TBL_WHITE_LEVEL_COMMISSION_SLAB on detail.DMR_SLAB equals white.SLN
                                    where detail.WHITE_LEVEL_ID == WLPInfo.UNDER_WHITE_LEVEL && detail.INTRODUCER_ID == MemberCurrentUser.MEM_ID && white.SLAB_TYPE == 3
                                    //where detail.WHITE_LEVEL_ID == MemberCurrentUser.MEM_ID  && detail.INTRODUCER_ID == 0 && white.SLAB_TYPE == 3
                                    select new
                                    {
                                        ID = dmrcom.SLN,
        ...
        public async Task<JsonResult> GenerateCommissionSlab(CommissoinManagmentmodel objval)
        {
            ...
            using (System.Data.Entity.DbContextTransaction ContextTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        var GetDistSlabCnt = await db.TBL_WHITE_LEVEL_COMMISSION_SLAB.Where(x => x.MEM_ID == MemberCurrentUser.MEM_ID).CountAsync();
                        if (GetDistSlabCnt < 11)
                        {
                            if (objval.Slab_TypeName == "MOBILE RECHARGE")
                            {
                                foreach (var item in objval.OperatorDetails)
                        ...
                        {
                            return Json(new { Result = "NotAllowed" });
                        }
