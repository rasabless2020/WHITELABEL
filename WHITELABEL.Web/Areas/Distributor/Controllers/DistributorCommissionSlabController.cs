...
{
    [Authorize]
    public class DistributorCommissionSlabController : DistributorBaseController
    {
        ...
        // GET: Distributor/DistributorCommissionSlab
        ...
        private IGrid<TBL_WHITE_LEVEL_COMMISSION_SLAB> CreateExportableGrid()
        {
            ...
            grid.Columns.Add(model => model.SLN).Titled("").Encoded(false).Filterable(true).Sortable(true)
                .RenderedAs(model => "<a href='" + @Url.Action("AddCommissionSlab", "DistributorCommissionSlab", new { area = "Distributor", SlabId = Encrypt.EncryptMe(model.SLN.ToString()) }) + "' class='btn btn-primary btn-xs'>Edit</a>");
            //grid.Columns.Add(model => model.SLN).Titled("").Encoded(false).Filterable(false).Sortable(false)
            // .RenderedAs(model => "<a href='javascript:void(0)' class='btn btn-denger btn-xs' onclick='DeactivateStatus(" + model.SLN + ");return 0;'>Deactivate</a>");
            ...
            //foreach (IGridColumn column in grid.Columns)   (model.ACTIVE_MEMBER == true ? "Deactive" : "Active")
            //{
        ...
        public ActionResult AddCommissionSlab(string SlabId = "")
        {
            if (Session["DistributorUserId"] != null)
            {
                ...
                {
                    Logger.Error("Controller:-  DistributorCommissionSlab(Distributor), method:- AddCommissionSlab (GET) Line No:- 197", ex);
        ...
        public JsonResult FetchData(long idval)
        {
            ...
            {
                Logger.Error("Controller:-  DistributorCommissionSlab(Distributor), method:- FetchData (GET) Line No:- 223", ex);
                throw;
            }
        ...
        public async Task<JsonResult> AutoComplete(string prefix)
        {
            ...
            {
                Logger.Error("Controller:-  DistributorCommissionSlab(Distributor), method:- AutoComplete (GET) Line No:- 429", ex);
                throw ex;
            }
        ...
        //public JsonResult AddCommissionSlab(dynamic objval)
        {
            ...
            using (System.Data.Entity.DbContextTransaction ContextTransaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        else if (objval.Slab_TypeName == "DMR")
                        {
                            ...
                            foreach (var item in objval.ServiceInformationDMR)
                            {
                                var checkvalid = db.TBL_COMM_SLAB_DMR_PAYMENT.Where(x => x.SLN == item.ID).FirstOrDefault();
                                if (Convert.ToDecimal(item.CommissionPercentage) > checkvalid.COMM_PERCENTAGE)
                                {
                                    return Json(new { Result = "Failure" });
                                    break;
                ...
                {
                    Logger.Error("Controller:-  DistributorCommissionSlab(Distributor), method:- AddCommissionSlab (POST) Line No:- 710", ex);
        ...
        public async Task<JsonResult> DeactiveOperator(string Id)
        {
            ...
            {
                Logger.Error("Controller:-  DistributorCommissionSlab(Distributor), method:- DeactiveOperator (POST) Line No:- 747", ex);
                throw;
            }
        }
        ...
        public JsonResult GetServiceProvider(string NewListId)
        {
            ...
            try
            {
                ...
                if (NewListId == "MOBILE RECHARGE")
                {
                    ...
                    var OperatorList = (from WList in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB
                                        ...
                                        where WList.INTRODUCER_ID == MemberCurrentUser.MEM_ID && WList.WHITE_LEVEL_ID == whitelevel.INTRODUCER && slbmst.SLAB_TYPE == 1
                                        select new
                                        {
                                            ID = commPO.SLN,
                ...
                else if (NewListId == "UTILITY SERVICES")
                {
                    ...
                    var OperatorList = (from WList in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB
                                        ...
                                        where WList.INTRODUCER_ID == MemberCurrentUser.MEM_ID && WList.WHITE_LEVEL_ID == whitelevel.INTRODUCER && slbmst.SLAB_TYPE == 2
                                        select new
                                        {
                                            ID = commPO.SLN,
                                            SERVICE_NAME = commPO.OPERATOR_NAME,
                ...
                else if (NewListId == "DMR")
                {
                    ...
                    var OperatorList = (from detail in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB
                                        ...
                                        where detail.WHITE_LEVEL_ID == whitelevel.INTRODUCER && detail.INTRODUCER_ID == MemberCurrentUser.MEM_ID && white.SLAB_TYPE == 3
                                        select new
                                        {
                                            ID = dmrcom.SLN,
                                            SERVICE_NAME = "",
                                            ...
                                            OperatorDetails = (from DMcm in db.TBL_COMM_SLAB_DMR_PAYMENT
                                                               ...
                                                               where dtlcom.WHITE_LEVEL_ID == whitelevel.INTRODUCER && dtlcom.INTRODUCER_ID == MemberCurrentUser.MEM_ID && DMcm.DMT_TYPE == "DOMESTIC"
                                            ...
                                            ServiceInformationDMR = (from DMcm in db.TBL_COMM_SLAB_DMR_PAYMENT
                                                                     join dtlcom in db.TBL_DETAILS_MEMBER_COMMISSION_SLAB on DMcm.SLAB_ID equals dtlcom.DMR_SLAB
                                                                     where dtlcom.WHITE_LEVEL_ID == whitelevel.INTRODUCER && dtlcom.INTRODUCER_ID == MemberCurrentUser.MEM_ID && DMcm.DMT_TYPE == "FOREIGN"
                                                                     select new CommissionListView
                                                                     {
                                                                         ID = DMcm.SLN,
                                                                         SERVICE_NAME = "Money Transfer (Nepal)",
            ...
            {
                Logger.Error("Controller:-  DistributorCommissionSlab(Distributor), method:- GetServiceProvider (POST) Line No:- 970", ex);
                throw ex;
            }
