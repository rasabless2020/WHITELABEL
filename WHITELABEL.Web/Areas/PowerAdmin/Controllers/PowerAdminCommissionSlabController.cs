...
{
    [Authorize]
    public class PowerAdminCommissionSlabController : PoweradminbaseController
    {
        private static readonly ILog Logger = LogManager.GetLogger(System.Environment.MachineName);
        // GET: PowerAdmin/PowerAdminCommissionSlab
        public void initpage()
        {
            try
            {
                ViewBag.ControllerName = "Power Admin Commission Slab";
        ...
        public PartialViewResult PoweradminComIndexGrid()
        {
            try
            {
        ...
        {
            var db = new DBContext();
            var CommissionSlab = db.TBL_WHITE_LEVEL_COMMISSION_SLAB.Where(x=>x.MEM_ID==0 && x.SLAB_STATUS==true).ToList();
            ...
            grid.Columns.Add(model => model.SLN).Titled("Action").Encoded(false)
                .RenderedAs(model => "<div style='text-align:center'><a href='" + @Url.Action("AddCommissionSlab", "PowerAdminCommissionSlab", new { area = "PowerAdmin", SlabId = Encrypt.EncryptMe(model.SLN.ToString()) }) + "' title='Edit'><i class='fa fa-edit'></i></a></div>");            
            ...
            try
            {
                ...
                if (NewListId == "MOBILE RECHARGE")
                {
                    ...
                    var OperatorList = (from x in db.TBL_SERVICE_PROVIDERS
                                            //where x.TYPE == operatortype || x.TYPE == operatortype1 || x.TYPE== OperatorTypeName
                                        where  x.TYPE.StartsWith(OperatorTypeName)
                                        select new
                                        {
                                            ID = x.ID,
