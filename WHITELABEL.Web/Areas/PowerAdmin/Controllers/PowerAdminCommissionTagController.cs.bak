    ...
    public class PowerAdminCommissionTagController : PoweradminbaseController
    {
        ...
        public async Task<JsonResult> MobileRechargeSlab()
        {
            try
            {
                ...
                using (DBContext context = new DBContext())
                {
                    var ret = await context.TBL_WHITE_LEVEL_COMMISSION_SLAB.Where(x => x.SLAB_TYPE == 1).Select(x => new { x.SLN, x.SLAB_NAME }).ToListAsync();
                    return Json(ret, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
        ...
        public async Task<JsonResult> UtilityRechargeSlab()
        {
            try
            {
                ...
                using (DBContext context = new DBContext())
                {
                    var ret = await context.TBL_WHITE_LEVEL_COMMISSION_SLAB.Where(x => x.SLAB_TYPE == 2).Select(x => new { x.SLN, x.SLAB_NAME }).ToListAsync();
                    return Json(ret, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
        ...
        public async Task<JsonResult> DMRRechargeSlab()
        {
            try
            {
                ...
                using (DBContext context = new DBContext())
                {
                    var ret = await context.TBL_WHITE_LEVEL_COMMISSION_SLAB.Where(x => x.SLAB_TYPE == 3).Select(x => new { x.SLN, x.SLAB_NAME }).ToListAsync();
                    return Json(ret, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
