...
{
    [Authorize]
    public class PowerAdminMemberController : PoweradminbaseController
    {
        ...
        // GET: PowerAdmin/PowerAdminMember
        ...
        public async Task<ActionResult> CreateMember(TBL_MASTER_MEMBER value, HttpPostedFileBase AadhaarFile, HttpPostedFileBase PanFile)
        {
            ...
            using (System.Data.Entity.DbContextTransaction ContextTransaction = db.Database.BeginTransaction())
            {
                    ...
                    if (CheckUser == null)
                    {
                        if (value.AADHAAR_NO != null || AadhaarFile != null)
                        {
                            if (AadhaarFile == null)
                            {
                                ...
                                var memberrole = await db.TBL_MASTER_MEMBER_ROLE.Where(x => x.ROLE_NAME == "WHITE LEVEL" || x.ROLE_NAME == "API USER").ToListAsync();
                                ViewBag.RoleDetails = new SelectList(memberrole, "ROLE_ID", "ROLE_NAME");
                        ...
                        if (value.PAN_NO != null || PanFile != null)
                        {
                            ...
                            {
                                ViewBag.checkstatus = "0";
                                ModelState.AddModelError("PAN_NO", "Please Upload Pan Card Image...");
                                var memberrole = await db.TBL_MASTER_MEMBER_ROLE.Where(x => x.ROLE_NAME == "WHITE LEVEL" || x.ROLE_NAME == "API USER").ToListAsync();
                                ViewBag.RoleDetails = new SelectList(memberrole, "ROLE_ID", "ROLE_NAME");
                                return View("CreateMember", value);
                            }
                            ...
                            {
                                ViewBag.checkstatus = "0";
                                ModelState.AddModelError("PAN_NO", "Please give Pan Card Number...");
                                var memberrole = await db.TBL_MASTER_MEMBER_ROLE.Where(x => x.ROLE_NAME == "WHITE LEVEL" || x.ROLE_NAME == "API USER").ToListAsync();
                                ViewBag.RoleDetails = new SelectList(memberrole, "ROLE_ID", "ROLE_NAME");
                                return View("CreateMember", value);
                            }
                        ...
                        value.INTRODUCER = CurrentUser.USER_ID;
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.Error("Controller:-  CreateMember(PowerAdmin), method:- CreateMember (POST) Line No:- 340", ex);
                    return RedirectToAction("Exception", "ErrorHandler", new { area = "" });
                    throw ex;
        ...
        public async Task<JsonResult> DeleteInformation(string id)
        {
            ...
            using (System.Data.Entity.DbContextTransaction ContextTransaction = context.Database.BeginTransaction())
            {
                ...
                {
                    Logger.Error("Controller:-  CreateMember(PowerAdmin), method:- DeleteInformation (POST) Line No:- 372", ex);
                    ContextTransaction.Rollback();
        ...
        public async Task<JsonResult> PasswordSendtoUser(string id)
        {
            ...
            {
                Logger.Error("Controller:-  CreateMember(PowerAdmin), method:- PasswordSendtoUser (POST) Line No:- 467", ex);
                return Json(new { Result = "false" });
            }
        ...
        private IGrid<TBL_MASTER_MEMBER> CreateExportableGrid()
        {
            ...
            grid.Columns.Add(model => model.MEM_ID).Titled("").Encoded(false).Filterable(false).Sortable(false).Titled("Action")
                 .RenderedAs(model => "<div class='btn-group btn-group-xs'><a href='javascript:void(0)' class='btn btn-denger' onclick='SendMailToMember(" + model.MEM_ID + ");return 0;'>Password</a></div>");
            grid.Columns.Add(model => model.SECURITY_PIN_MD5).Titled("Security Pin").Filterable(true).Sortable(true);
            ...
            grid.Columns.Add(model => model.MEM_ID).Titled("").Encoded(false).Filterable(false).Sortable(false).Titled("Action")                        
                        .RenderedAs(model => "<input type'checkbox' id='toggle-two'>");
