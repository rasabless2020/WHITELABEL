...
{
    [Authorize]
    public class SuperBankDetailsController : SuperBaseController
    {
        ...
        public ActionResult BankDetails(string Bankid = "")
        {
            ...
            if (Session["SuperDistributorId"] != null)
            {
                ...
                {
                    Logger.Error("Controller:-  SuperBankDetails(Super), method:- BankDetails (GET) Line No:- 159", ex);
        ...
        private IGrid<TBL_SETTINGS_BANK_DETAILS> CreateExportableGrid()
        {
            try
            {
                ...
                grid.Columns.Add(model => (model.ISDELETED == 1 ? "Deactive" : "Active")).Titled("Status").Filterable(true).Sortable(true);
                grid.Columns.Add(model => model.SL_NO).Titled("Edit").Encoded(false).Filterable(false).Sortable(false)
                //.RenderedAs(model => "<a href='" + @Url.Action("BankDetails", "SuperBankDetails", new { area = "Super", Bankid = Encrypt.EncryptMe(model.SL_NO.ToString()) }) + "' class='btn btn-primary btn-xs'>Edit</a>");
                .RenderedAs(model => "<div style='text-align:center;'><a href='" + @Url.Action("BankDetails", "SuperBankDetails", new { area = "Super", Bankid = Encrypt.EncryptMe(model.SL_NO.ToString()) }) + "' title='Edit'><i class='fa fa-edit'></i></a></div>");
                
                grid.Columns.Add(model => model.SL_NO).Titled("Action").Encoded(false).Filterable(false).Sortable(false)
                    //.RenderedAs(model => "<a href='javascript:void(0)' onclick='DeActivateBank(" + model.SL_NO + ");return 0;'></a>");
