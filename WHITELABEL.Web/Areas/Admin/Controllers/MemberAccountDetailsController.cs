...
{
    [Authorize]
    public class MemberAccountDetailsController : AdminBaseController
    {
        ...
        // GET: Admin/MemberAccountDetails
        ...
        private IGrid<TBL_ACCOUNTS> CreateExportableGrid()
        {
            ...
            var accountstatement = (from mem in db.TBL_MASTER_MEMBER
                                    join ac in db.TBL_ACCOUNTS on mem.MEM_ID equals ac.MEM_ID
                                    //join role in db.TBL_MASTER_MEMBER_ROLE on mem.MEMBER_ROLE equals role.ROLE_ID where mem.CREATED_BY==MemberCurrentUser.MEM_ID
                                    join role in db.TBL_MASTER_MEMBER_ROLE on mem.MEMBER_ROLE equals role.ROLE_ID
                                    where mem.INTRODUCER == MemberCurrentUser.MEM_ID
                                    select new
                                    {
                                        sln = ac.ACC_NO,
                                        username = mem.UName,
                                    ...
                                    }).AsEnumerable().Select(z => new TBL_ACCOUNTS()
                                    {
                                        ...
                                        GST=z.GST,
                                        TDS=z.TDS
                                    }).ToList().OrderByDescending(x=>x.TRANSACTION_TIME);


            IGrid<TBL_ACCOUNTS> grid = new Grid<TBL_ACCOUNTS>(accountstatement);
            grid.ViewContext = new ViewContext { HttpContext = HttpContext };
            grid.Query = Request.QueryString;
            grid.Columns.Add(model => model.ACC_NO).Titled("Sln");
