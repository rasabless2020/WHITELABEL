        ...
        public PartialViewResult indexgrid(string DateFrom="",string Date_To="")
        {
            ...
            if(DateFrom != ""&& Date_To != "")
            {
                ...
                var listaccount = (from x in db.TBL_ACCOUNTS
                                   ...
                                   where y.INTRODUCER == MemberCurrentUser.MEM_ID && x.MEMBER_TYPE == "DISTRIBUTOR" && (x.TRANSACTION_TYPE != "Mobile Recharge" && x.TRANSACTION_TYPE != "DMR") && x.TRANSACTION_DATE>= Date_From_Val && x.TRANSACTION_DATE<= Date_To_Val
                                   select new
                                   {
                                       UserName = y.UName,
                                       MEM_ID = x.MEM_ID,
            ...
            else
            {
                ...
                var listaccount = (from x in db.TBL_ACCOUNTS
                                   join y in db.TBL_MASTER_MEMBER on x.MEM_ID equals y.MEM_ID
                                   where y.INTRODUCER == MemberCurrentUser.MEM_ID && x.MEMBER_TYPE == "DISTRIBUTOR" && (x.TRANSACTION_TYPE != "Mobile Recharge" && x.TRANSACTION_TYPE != "DMR") && x.TRANSACTION_DATE == Todaydate
                                   select new
                                   {
                                       UserName = y.UName,
                                       MEM_ID = x.MEM_ID,
