...
{
    [Authorize]
    public class MemberGSTCalculationController : AdminBaseController
    {
        //public void initpage()
        //{
        //    try
        //    {
        //        ViewBag.ControllerName = "Member Account Details";
        ...
        // GET: Admin/MemberGSTCalculation
        ...
        public JsonResult CalculateGSTAmount(string MonthID = "", string ServiceName = "", string Year = "")
        {
            ...
            if (MonthID != null)
            {
                ...
                var GSTValu12e = (from x in db.TBL_ACCOUNTS
                                  where x.MEM_ID == MemberCurrentUser.MEM_ID && x.TRANSACTION_DATE.Month == Monthval && x.DR_CR == "CR" && x.TRANSACTION_TYPE == "Mobile Recharge" && x.SERVICE_ID == serviceid && x.MEMBER_TYPE == "WHITE LEVEL" && x.NARRATION == "Commission"
                                  select new
                                  {
                                      MEM_ID = x.MEM_ID,
                ...
                var getListGSTComm = db.TBL_ACCOUNTS.Where(x => x.MEM_ID == MemberCurrentUser.MEM_ID && x.TRANSACTION_DATE.Month == Monthval && x.DR_CR == "CR" && x.TRANSACTION_TYPE == "Mobile Recharge" && x.SERVICE_ID == serviceid && x.MEMBER_TYPE == "WHITE LEVEL" && x.NARRATION == "Commission").FirstOrDefault();
